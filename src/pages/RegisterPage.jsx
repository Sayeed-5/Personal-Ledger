    ...
    return (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'var(--bg-dark)', minHeight: 'calc(100vh - 57px)' }}>
            <div className="ledger-card anim-fade" style={{ width: '100%', maxWidth: 400, padding: 32 }}>
                ...
                <h1 style={{ fontSize: '1.75rem', fontWeight: 700, marginBottom: 4, color: 'var(--text)' }}>Create account</h1>
                <p style={{ color: 'var(--text-dim)', fontSize: '0.95rem', marginBottom: 24 }}>Start tracking your debts for free</p>

                {/* ── Google button ── */}
                <GoogleButton onClick={handleGoogle} loading={gLoading} label={gLoading ? 'Signing in…' : 'Sign up with Google'} />
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                        <label htmlFor="rn" style={{ fontSize: '0.88rem', color: 'var(--text-dim)', fontWeight: 500 }}>Full Name</label>
                        <input id="rn" type="text" placeholder="Sayeed Ahemmad" required autoComplete="name"
                            value={fullName} onChange={(e) => setFullName(e.target.value)} className="ledger-input" />
                    </div>

                    {/* Email */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                        <label htmlFor="re" style={{ fontSize: '0.88rem', color: 'var(--text-dim)', fontWeight: 500 }}>Email</label>
                        <input id="re" type="email" placeholder="you@example.com" required autoComplete="email"
                            value={email} onChange={(e) => setEmail(e.target.value)} className="ledger-input" />
                    </div>

                    {/* Password */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                        <label htmlFor="rp" style={{ fontSize: '0.88rem', color: 'var(--text-dim)', fontWeight: 500 }}>
                            Password
                            <span style={{ color: 'var(--text-dim)', fontWeight: 400, fontSize: '0.8rem', marginLeft: 8 }}>(min. 6 characters)</span>
                        </label>
                        <input id="rp" type="password" placeholder="••••••••" required autoComplete="new-password"
